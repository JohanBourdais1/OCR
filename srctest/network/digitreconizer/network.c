...
// Fonction pour détecter une entrée clavier sans bloquer
// Utilise read() directement au lieu de getchar() pour plus de fiabilité
...
// AMÉLIORATION: Batch Normalization simple
// Normalise: (x - mean) / sqrt(var + epsilon)
void batch_norm(double* data, size_t size, double epsilon)
{
    ...
    // Normaliser les données: (x - mean) / sqrt(var + epsilon)
                ...
                double* filter, double* input, double* biais, double *conv_out)
{
    ...
    // AMÉLIORATION: Batch Normalization après ReLU
    batch_norm(conv_out, nb_out * out_size * out_size, 1e-5);
}
...
void dense_reLU(network* net, double* input)
{
    ...
    // AMÉLIORATION: Batch Normalization après ReLU
    batch_norm(net->hiddenValues, HIDDEN_SIZE, 1e-5);
}
...
int image_to_array(char* path, double** array)
{
    ...
    // AMÉLIORATION: Normalisation min-max pour meilleur contraste
    double gray_values[784];
    double min_val = 255.0, max_val = 0.0;
    
    // Premier pass: convertir en grayscale et trouver min/max
    ...
    // Deuxième pass: normaliser avec min-max scaling
    if (max_val > min_val) {
        for (size_t i = 0; i < 784; i++) {
            input[i] = (gray_values[i] - min_val) / (max_val - min_val);
        }
    } else {
        // Image uniforme - normaliser simplement
        for (size_t i = 0; i < 784; i++) {
...
int image_to_array_inverted(char* path, double** array)
{
    ...
    if (debug_surface != NULL) {
        ...
        sprintf(array, "../data/pythonData/grille/%c%c.png", path[10], path[11]);
        IMG_SavePNG(debug_surface, array);
        IMG_SavePNG(debug_surface, t);
...
{
    DIR* directory = opendir("../data/pythonData/test/grille");
    if (directory == NULL) {
        fprintf(stderr, "Can't open ../data/pythonData/test/grille\n");
        return;
    }

    struct dirent* entry = NULL;
    
    printf("\n=== TESTING GRILLE IMAGES ===\n\n");
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        snprintf(full_name, MAX_FILE_NAME_SIZE, "../data/pythonData/test/grille/%s", entry->d_name);
...
{
    double lr = 0.001f;  // AMÉLIORATION: augmenté de 0.0006 à 0.001 pour convergence plus rapide
    ...
    // AMÉLIORATION: ReduceLROnPlateau adaptatif
    double best_loss = 1e9;
    int plateau_count = 0;
    int plateau_patience = 5000;  // Réduire LR après 5k itérations sans amélioration
    double loss_improvement_threshold = 0.9995;  // 0.05% d'amélioration minimum
    int lr_reductions = 0;
    ...
    struct dirent* entry = NULL;
    int e = 0;
    int user_quit = 0;  // Flag pour détecter si l'utilisateur appuie sur 'q'
    
        while ((entry = readdir(directory)) != NULL && e < max_iterations && !user_quit) {
            // Vérifier si l'utilisateur appuie sur 'q' pour quitter (tous les 500 itérations pour éviter la saturation)
            if (e % 500 == 0) {
                int key = kbhit_nonblocking();
                if (key == 'q' || key == 'Q') {
                    printf("\n\n[USER INTERRUPT] Training stopped at iteration %d\n", e);
                    user_quit = 1;
                    break;
                }
            }
            ...
            if (entry->d_type == DT_DIR) {
                ...
                // AMÉLIORATION: Calculer la cross-entropy loss pour ReduceLROnPlateau
                double current_loss = -log(fmax(n->outputValues[gt], 1e-10));
                
                printf("%d    %d    %f    %d\n\n", gt, pred, n->outputValues[pred], e);
                
                // AMÉLIORATION: Adapter le learning rate si loss stagne
                if (current_loss < best_loss * loss_improvement_threshold) {
                    best_loss = current_loss;
                    plateau_count = 0;
                } else {
                    plateau_count++;
                    if (plateau_count >= plateau_patience && lr > 1e-6) {
                        lr *= 0.5;
                        lr_reductions++;
                        plateau_count = 0;
                        printf("\n[LR ADAPTED] Reduced to %.2e (reduction #%d at iter %d)\n\n", lr, lr_reductions, e);
                    }
                }
                ...
                // AMÉLIORATION: Utiliser le LR adaptatif directement (ReduceLROnPlateau)

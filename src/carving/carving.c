...
{
    if (argc < 2)
        errx(EXIT_FAILURE, "You have to specify an image path !");

    if (argc > 2)
        errx(EXIT_FAILURE, "You have to type ./carving random_image.png");
    ...
    if (system("rm -rf grid") != 0)
        errx(EXIT_FAILURE, "/!\\ system(\"rm -rf\") failure");

    // Image loading
    SDL_Surface* source;
    if ((source = IMG_Load(argv[1])) == NULL)
    ...
    carve(source, intersections);

    // Creations of the tiles of the grid in "grid/".
    cut(source, intersections);
